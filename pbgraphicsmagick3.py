...
def change_size(filename, insizes):
	for size in sizes:
		cmd = 'convert -size '+ size +' ' + filename+'.jpg -resize '+ size +' ' + filename+'_'+size+'.jpg'
		shellcmd(cmd) 
...
def display_4up(filename, size=print_size):
	yloc = ['+124+12 ', '+124+373 ', '+652+373 ', '+652+12 ']
        for i in range(4):
		cmd = 'composite -resize 504x336 -geometry  ' 
		cmd = cmd + yloc[i] + filename + '_' + suffix[i] + '.jpg '
		if i==0: cmd = cmd + ' images/backscreen.jpg'
		else: cmd = cmd + ' boutput.jpg'
        	cmd = cmd + ' boutput.jpg'
	        shellcmd(cmd)
	shellcmd('composite -geometry +340+210 -resize 600x images/overlay.png  boutput.jpg ' + filename + '_display.jpg')


def print_4up(filename, size=print_size):	
	yloc = ['+60+498 ', '+60+1124 ', '+60+1750 ', '+60+2376 ']
        for i in range(4):
		cmd = 'composite -resize 878x582 -geometry ' 
		cmd = cmd + yloc[i] + filename + '_' + suffix[i] + '.jpg '
		if i==0: cmd = cmd + ' images/background-half-rot.jpg'
		else: cmd = cmd + ' output.jpg'
        	cmd = cmd + ' output.jpg'
	        shellcmd(cmd)
	shellcmd('composite -geometry +200+150 -resize 1000x images/overlay.png  output.jpg ' + filename + '_phone.jpg')

	shellcmd('composite -geometry +0+0 ' + filename + '_phone.jpg images/background-rot.jpg -quality 100 done.jpg')
	shellcmd('composite -geometry +1001+0 ' + filename + '_phone.jpg done.jpg -quality 100 done.jpg')
	shellcmd('convert -stroke gray -draw "line 1000,0 1000,3000" done.jpg -quality 90 ' + filename + '_print.jpg')
