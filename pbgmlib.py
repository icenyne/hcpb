...
def grab_image(filename, i, quiet=True, process=False):
	if not(quiet): 
		print
		print 'Count down: 3...'
		time.sleep(0.25)
	...
	if i in range(4): C.capture_image(filename+'_'+suffix[i] + '.jpg')
...
# function to make full HD (1980x1080) display 4-tile-up
...
def print_4upbigall(filename, i):
	...
	move_files(filename, path='', copy=False)
